...
import { LawDocumentModal } from './LawDocumentModal'
...
export function LawCard({ norma, index = 0 }: Props) {
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex gap-2 mt-auto">
          <button
            ...
            className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg border border-navy-200 bg-navy-50 text-navy-800 text-xs font-semibold hover:bg-navy-100 transition-colors cursor-pointer"
          >
      ...
      {showModal && (
        <LawDocumentModal norma={norma} onClose={() => setShowModal(false)} />
      )}
    </>
  )
}
