...
import { startTrial as startTrialService } from "@/lib/plans";
...
const fetchPlan = useCallback(async () => {
  ...
  setLoading(true);
  try {
    const { data: subRaw } = await supabaseClient
      .from("subscriptions")
      .select("*")
      .eq("user_id", user.id)
      .single();
    ...
    if (sub?.plan === "firma") {
      ...
      const { data: memberRaw } = await supabaseClient
        .from("firm_members")
      ...
      const member = memberRaw as MemberWithFirm | null;

      if (member) {
        setFirm(member.firms ?? null);
        setFirmRole(member.rol_firma);
      }
    } else {
      setFirm(null);
      setFirmRole(null);
  ...
  const startTrial = useCallback(async () => {
    ...
    try {
      await startTrialService(user.id);
      await fetchPlan();
    } catch (error) {
      console.error("Error al iniciar el trial:", error);
    }
  }, [user, plan, fetchPlan]);
...
export function usePlanLimits() {
  const { plan, limits, can, canCreateCase, canCreateNote, loading } =
    usePlan();
